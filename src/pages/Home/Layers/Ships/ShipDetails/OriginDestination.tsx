...
const OriginDestination = ({ etaHours }: { etaHours: number }) => {
  const { selectedShip } = useAppContext();

  const properties = selectedShip.feature.properties || {};
  return (
    <div className="w-full flex flex-col items-center justify-center gap-2">
      <div className="w-full mt-6 p-4 bg-gray-100 rounded-lg shadow-inner flex items-center justify-center space-x-4 select-none text-gray-800 font-semibold text-sm">
      ...
      {etaHours <= 0 && <p className="text-sm">Arrived</p>}
    </div>
  );
};
